...
/** 升星消耗碎片: 按稀有度与当前星级计算 (star 为升级前星级 0-4) */
export function starUpFragmentCost(rarity: HeroRarity, star: number): number {
  if (rarity === "rare") return 50 * (star + 1);
  if (rarity === "epic") return 100 * (star + 1);
  return 100 + 300 * star;
}
